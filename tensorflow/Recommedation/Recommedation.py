...
ratings = pd.read_csv('data.csv', sep=',', names=ratings_cols, encoding='utf-8')
...
def build_rating_sparse_tensor(ratings_df):
    ...
    return tf.SparseTensor(
        indices=indices,
        values=values,
        dense_shape=[users.shape[0], contents.shape[0]])
...
model = build_model(ratings, embedding_dim=15, init_stddev=0.1)
...
model_lowinit = build_model(ratings, embedding_dim=15, init_stddev=0.01)
