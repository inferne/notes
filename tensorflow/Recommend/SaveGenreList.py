...
if rdscf is None:
    ...
    exit()
# rdscf = project_conf['pika'][cluster]['servers']
# print(rdscf)
# exit()
...
for index, row in genre.iterrows():
    ...
    rl = model.user_recommendations(reg_model, model.DOT, exclude_rated=False, k=topk, id=index)
    rl = rl.merge(contents, on="content_id", how="left")
    dt = {int(rl.loc[i, 'id']):rl.loc[i, 'dot score'] for i in rl.index}
    # print(dt)
