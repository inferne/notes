 ...
 */
int
recv_fd_by_stream(int fd, ssize_t (*userfunc)(int, const void *, size_t))
{
 ...
 */
int recv_fd_by_unix_socket(int fd, ssize_t (*userfunc)(int, const void *, size_t))
{
    int              newfd, nr, status;
    char            *ptr;
    char             buf[MAXLINE];
    ...
    for ( ; ; ) {
        ...
        /* 
         * See if this is the final data with null & status.
         * Null is next to last byte of buffer, status
         * byte is last byte. Zero status means there is
         * a file descriptor to receive.
         */
